...
<body>
	...
	<p> Якщо дані таблиці відстуні, значить відсутні дані в таблиці. </p>
    ...
    if($result = $mysqli->query('SELECT * FROM company')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит

        printf("Список студентів: <br><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>department_ID_worker</th><th>TaxCode</th><th>Title</th><th>ID_comp</th></tr>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['department_ID_worker'], $row['TaxCode'], $row['Title'], $row['ID_comp']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
    ...
    <ul>
        ...
        <li><a href="deleteFromCompany.php">Видалити рядок</a><br></li>
