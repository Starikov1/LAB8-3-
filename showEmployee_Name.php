...
<head>
    ...
    <title>Таблиця Employee_Name</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <h1>Таблиця Employee_Name</h1>
    ...
    if($result = $mysqli->query('SELECT * FROM Employee_Name')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['Passport'], $row['Name'], $row['PDirector']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
    ...
    <ul>
        <li><a href="insertIntoEmployee_NameForm.php">Вставити рядок</a><br></li>
