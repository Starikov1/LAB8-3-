...
<body>
    ...
    <form action="insertIntoEO.php" method="post">
        <label>Passport</label><input name="Passport" type="text"><br>
        <label>TaxCode</label><input name="TaxCode" type="text"><br>
		<label>director_Passport_id</label><input name="director_Passport_id" type="text"><br>
		<label>director_department_id</label><input name="director_department_id" type="text"><br>
	...
	<?php
		...
		if($mysqli->query("INSERT INTO Director_Other(Passport,TaxCode,director_Passport_id,director_department_id) VALUES('$Passport','$TaxCode', '$director_Passport_id', '$director_department_id');")){};
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
			...
			<td>%s</td>
			<td>%s</td>
<td>%s</td></tr>", $row['Passport'], $row['TaxCode'], $row['department_id_worker'], $row['director_department_id']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
